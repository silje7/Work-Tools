...
1. Normal scan with optimized file size:
   python webscreengrab4.py ips.txt --local-chromedriver "c:\\path\\to\\chromedriver.exe" --max-content-size 500 --screenshot-quality 40
   
python webscreengrab4.py ips.txt --local-chromedriver "c:\\users\\v613867\\desktop\\projects\\tools\\chromedriver-win64\\chromedriver.exe" --max-content-size 500 --screenshot-quality 40 --use-jpg-screenshots --store-minimal-json --compression --output-excel BatchE.xlsx
        ...
        if row_data["screenshot_path"] and not args.screenshots_external:
            try:
                if os.path.exists(row_data["screenshot_path"]):
                    ...
                    max_width = args.screenshot_max_size if args.screenshot_max_size > 0 else 20
                    ...
                    ws.row_dimensions[row_num].height = max(row_height, 200)  # Reduced from 180
